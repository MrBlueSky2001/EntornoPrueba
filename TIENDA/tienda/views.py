...
from .forms import CompraForm, ClienteRegistrationForm, CheckoutForm, CarritoForm
...
class ListadoComprarFilterView(FilterView):
    ...
    filterset_class = TiendaListFilter
    context_object_name = "productos"

    def get(self, request, *args, **kwargs):
        queryset = Producto.objects.all().filter(unidades__gt=0)
        user = request.user
        if user.is_authenticated and client_check(user):
        ...
        context['productos'] = self.object_list
...
class ProductoDetailView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not request.user.is_anonymous and client_check(request.user):
...
# class ProductoDetailView(FormMixin, DetailView):
#     model = Producto
#     form_class = CompraForm
#     template_name = 'tienda/user/compra/detalle_producto.html'
#
#     def get_initial(self):
#         return {'id_producto': self.object.id}
#
#     def get_context_data(self, **kwargs):
#         context = super().get_context_data(**kwargs)
#         if not self.request.user.is_anonymous and client_check(self.request.user):
#             context['valoraciones'] = Valoracion.objects.filter(productocompra__producto_id=self.object.id)
#         return context
...
class ComprobarCheckoutMixin:
    def setup(self, request, *args, **kwargs):
        ...
        if 'lista_productos_carrito' not in self.request.session:
            ...
            self.redirect_to_tienda()

    def redirect_to_tienda(self):
        return HttpResponseRedirect(reverse_lazy('tienda'))

class CheckoutCreateView(ComprobarCheckoutMixin, LoginRequiredMixin, UserPassesTestMixin, FormView):
    ...
    def get_form(self, form_class=None):
        ...
        form.fields['direccion_envio'].queryset = Direccion.objects.filter(cliente__user_id=self.request.user.id,
                                                                           envio=True)
        form.fields['direccion_facturacion'].queryset = Direccion.objects.filter(cliente__user_id=self.request.user.id,
                                                                                 facturacion=True)
        return form
    ...
    def form_valid(self, form):
        ...
        print("Productos compra: ", productoscompra)
...
class ClienteDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # if not client_check(self.object.user):
...
class CompraDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def test_func(self):
        ...
        if client_check(self.request.user) and compra.cliente.user.id == self.request.user.id:
            result = True
        return result
